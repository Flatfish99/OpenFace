...
std::string set_data( double time_stamp,double landmark_confidence,const cv::Vec2f& gaze_angle,const cv::Vec6f& pose_estimate)
{
    char data[300];
   
     sprintf(data,"\"time_stamp\":%f,\"landmark_confidence\":%f,\"gaze_angle_x\":%f,\"gaze_angle_y\":%f,\"pose_Tx\":%f,\"pose_Ty\":%f,\"pose_Tz\":%f,\"pose_Rx\":%f,\"pose_Ry\":%f,\"pose_Rz\":%f}",time_stamp,landmark_confidence,gaze_angle[0],gaze_angle[1],pose_estimate[0],pose_estimate[1],pose_estimate[2],pose_estimate[3],pose_estimate[4],pose_estimate[5]);
...
//             send the results by 0mq
            data =set_data( sequence_reader.time_stamp, face_model.detection_certainty,gazeAngle,pose_estimate);
